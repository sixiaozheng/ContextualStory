...
from contextualstory.utils.util import save_videos_grid, compute_snr
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    unet            = VideoLDMUNet3DConditionModel.from_pretrained(
        ...
        use_temporal=OmegaConf.to_container(unet_additional_kwargs['use_temporal'], resolve=True), #True if not is_image else False,
        ...
        use_motion_condition=unet_additional_kwargs['use_motion_condition'],
        use_text_cross_attention=use_text_cross_attention,
        num_layers_FStext=num_layers_FStext,
        zero_init_storyflow_adapter=zero_init_storyflow_adapter,
        zero_init_FSText=zero_init_FSText,
        FStext_layer_depth=FStext_layer_depth,
        FStext_layer_temporal=OmegaConf.to_container(FStext_layer_temporal, resolve=True),
        FStext_layer_causal=OmegaConf.to_container(FStext_layer_causal, resolve=True),
        # use_safetensors=True
    )

    accelerator.print(f"{unet.text_adapter}")
    ...
    if train_data['dataset'] == "pororo":
        train_dataset = PororoDataset(**train_data)
    elif train_data['dataset'] == "flintstones":
        train_dataset = FlintstonesDataset(**train_data)
    else:
    ...
    logger.info("***** Some important setting *****")
    if unet_additional_kwargs['first_frame_condition_mode']=="none":
        logger.info("Story Visualization")
    else:
        logger.info("Story Continuation")
    logger.info(f"  fine-tune temporal_attn: {unet_additional_kwargs['use_temporal']['temporal_attn']}")
    logger.info(f"  fine-tune temporal_conv: {unet_additional_kwargs['use_temporal']['temporal_conv']}")
    logger.info(f"  Spatially-Enhanced Temporal Attention: {unet_additional_kwargs['augment_temporal_attention']}")
    logger.info(f"  first_frame_condition_mode: {unet_additional_kwargs['first_frame_condition_mode']}")
    logger.info(f"  StoryFlow Adapter: {unet_additional_kwargs['use_motion_condition']}")
    logger.info(f"  zero_init_storyflow_adapter: {zero_init_storyflow_adapter}")
    logger.info(f"  Text Adapter: {use_text_cross_attention}")
    logger.info(f"  num_layers_Storyline_Contextualizer: {num_layers_FStext}")
    logger.info(f"  zero_init_Storyline_Contextualizer: {zero_init_FSText}")
    

    logger.info("***** Running training *****")
    logger.info(f"  Dataset: {train_data['dataset']}")
    ...
    if resume_from_checkpoint is not None:
        ...
        if unet_additional_kwargs["use_motion_condition"]:
            with open(os.path.join(resume_from_checkpoint ,"cumulative_motion.json"), 'r') as f:
                cumulative_motion = json.load(f)
                cumulative_motion = torch.tensor(cumulative_motion, dtype=torch.float16, device=accelerator.device) # [1, 31]
    else:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.is_main_process and epoch == first_epoch and step == 0:
                ...
                for idx, (pixel_value, text) in enumerate(zip(pixel_values, texts)):
                    ...
                    save_videos_grid(pixel_value, f"{output_dir}/sanity_check/{f'no_text-{idx}'}.gif", rescale=True)
            ...
            with accelerator.accumulate(unet):
                ...
                if cfg_random_null_img_ratio > 0.0 and unet_additional_kwargs["first_frame_condition_mode"] != "none":
                ...
                if cfg_random_null_text_ratio > 0.0:
                    is_null_text = [False if random.random() > cfg_random_null_text_ratio else True for name in batch['text']]

                null_text = [""]*video_length
                prompt_ids = tokenizer(null_text, max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt").input_ids.to(latents.device)
                null_text_encoder_hidden_states = text_encoder(prompt_ids)[0]

                batch['text'] = list(chain(*batch["text"]))
                prompt_ids = tokenizer(
                    batch['text'], max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
                ).input_ids.to(latents.device)

                text_embedding = text_encoder(prompt_ids)
                last_hidden_state = text_embedding['last_hidden_state']
                pooler_output = text_embedding['pooler_output']

                encoder_hidden_states = (last_hidden_state, pooler_output, null_text_encoder_hidden_states, is_null_text)
                ...
                timesteps_rp = repeat(timesteps, "b -> b f", f=video_length)
                timesteps_rp = rearrange(timesteps_rp, "b f -> (b f)")
                ...
                motion=None
                if unet_additional_kwargs["use_motion_condition"]:
                    latents_for_diff = rearrange(latents, "b c f h w -> b f c h w")
                    diff_latents = latents_for_diff[:, 1:, :, :, :] - latents_for_diff[:, :-1, :, :, :]
                    diff_latents = rearrange(diff_latents, "b f c h w -> b f c (h w)")

                    motion = torch.norm(diff_latents, dim=(2,3)) # [6,4]
                    motion_mean = motion.mean(dim=0, keepdim=True)  # Calculate the mean along the first dimension
                    if step == 0:
                        cumulative_motion = motion_mean
                    else:
                        cumulative_motion = (cumulative_motion * step + motion_mean) / (step + 1)  # Calculate the cumulative average
                    motion = motion.long()
                ...
                if unet_additional_kwargs["first_frame_condition_mode"] != "none":
                    model_pred = unet(noisy_latents, timesteps_rp, encoder_hidden_states, first_frame_latents=first_frame_latents, frame_stride=frame_stride, motion_cond=motion).sample
                    target = target[:, :, 1:, :, :]
                else:
                    model_pred = unet(noisy_latents, timesteps_rp, encoder_hidden_states, motion_cond=motion).sample
                ...
                if snr_gamma is None:
                    ...
                    # logger.info("Computing SNR loss")
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        if unet_additional_kwargs["use_motion_condition"]:
                            with open(f"{save_path}/cumulative_motion.json", 'w') as f:
                                json.dump(cumulative_motion.tolist(), f)
                            print(cumulative_motion.tolist())
                ...
                if accelerator.is_main_process and global_step % validation_steps == 0:
                    ...
                    prompts = OmegaConf.to_container(validation_data.prompts)
    ...
    if accelerator.is_main_process:
        ...
        if unet_additional_kwargs["use_motion_condition"]:
            with open(f"{output_dir}/final_checkpoint/cumulative_motion.json", 'w') as f:
                json.dump(cumulative_motion.tolist(), f)
            print(cumulative_motion.tolist())

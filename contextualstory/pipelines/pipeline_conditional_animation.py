...
class ConditionalAnimationPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(self, prompt, device, num_videos_per_prompt, do_classifier_free_guidance, negative_prompt, video_length):
        ...
        text_embeddings_pooled = text_embeddings["pooler_output"]
        ...
        #     # print("use_text_cross_attention")
        #     weight_dtype = text_embeddings.dtype
        #     mask = ~torch.eye(video_length, dtype=bool, device=device)
        #     mask = mask.repeat_interleave(text_embeddings.shape[-2], dim=0)
        #     mask = mask.repeat_interleave(text_embeddings.shape[-2], dim=1)
        #     mask = mask.unsqueeze(0).expand(batch_size//video_length, -1, -1)
        #     text_embeddings = rearrange(text_embeddings, "(b f) n c -> b (f n) c", f=video_length)
                
        #     text_embeddings_ca = text_adapter(text_embeddings, text_embeddings, attention_mask=mask)
        #     text_embeddings = rearrange(text_embeddings_ca, "b (f n) c -> (b f) n c", f=video_length)
        ...
        return text_embeddings, text_embeddings_pooled
    ...
    def check_inputs(self, prompt, height, width, callback_steps, first_frame_paths=None):
        ...
        if first_frame_paths is not None and not isinstance(first_frame_paths, str) and not isinstance(first_frame_paths, list):
    ...
    def __call__(
        ...
        # text_adapter: Optional[Callable] = None,
        ...
        **kwargs,
        ...
        if isinstance(prompt, list):
            batch_size = len(prompt)//video_length
            first_frame_input = first_frame_paths if first_frame_paths is not None else first_frames
            if first_frame_input is not None and isinstance(first_frame_input, list):
                assert batch_size == len(first_frame_input), "prompt and first_frame_paths should have the same length"
        ...
        text_embeddings, text_embeddings_pooled = self._encode_prompt(
            prompt, device, num_videos_per_prompt, do_classifier_free_guidance, negative_prompt, video_length
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if first_frame_latents is not None:
                    ...
                    noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=(text_embeddings, text_embeddings_pooled), first_frame_latents=first_frame_latents_input, frame_stride=frame_stride, motion_cond=motion_cond, mode="test", do_classifier_free_guidance=do_classifier_free_guidance).sample.to(dtype=latents_dtype)
                else:
                    noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=(text_embeddings, text_embeddings_pooled), motion_cond=motion_cond, mode="test", do_classifier_free_guidance=do_classifier_free_guidance).sample.to(dtype=latents_dtype)
                # noise_pred = []
                # import pdb
                # pdb.set_trace()

...
class VideoLDMCrossAttnDownBlock(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            if self.use_temporal["temporal_conv"]:
                conv3ds.append(
                    TemporalResnetBlock(
                        in_channels=out_channels,
                        out_channels=out_channels,
            ...
            if self.use_temporal["temporal_attn"]:
                tempo_attns.append(
                    Transformer2DConditionModel(
                        n_temp_heads,
                        ...
                        in_channels=out_channels,
                        num_layers=transformer_layers_per_block,
                        cross_attention_dim=None, #cross_attention_dim, # Do not input storyline embedding to temporal attention
                        norm_num_groups=resnet_groups,
                        use_linear_projection=use_linear_projection,
                        only_cross_attention=only_cross_attention,
                        double_self_attention=True, # self-attention + self-attention
                        upcast_attention=upcast_attention,
                        attention_type=attention_type,
                        # additional
                        n_frames=n_frames,
    ...
    def forward(
        ...
        condition_on_first_frame = self.first_frame_condition_mode == "concat" 
...
class VideoLDMUNetMidBlock2DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        for attn, tempo_attn, resnet, conv3d in zip(self.attentions, self.tempo_attns, self.resnets[1:], self.conv3ds[1:]):

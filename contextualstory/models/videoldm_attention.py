...
class TemporalConditionalAttention(Attention):
    ...
    def forward(
        ...
        **cross_attention_kwargs):
        ...
        hidden_states = rearrange(hidden_states, '(b t) hw c -> b hw t c', t=self.n_frames) # [3, 1024, 5, 320]
        ...
        if encoder_hidden_states is not None:
            ...
            encoder_hidden_states = rearrange(encoder_hidden_states, '(b t) c h w -> (b h w) t c', t=self.n_frames-1)
            ...
            # # TODO: access sentence information of other images，(b hw) n*t=77*5 c

        if adjacent_slices is not None:
            assert encoder_hidden_states is None
            # adjacent_slices = rearrange(adjacent_slices, 'b c h w n -> b (h w) n c')
            adjacent_slices = rearrange(adjacent_slices, '(b t) c h w n -> b (h w) (t n) c', t=self.n_frames)
            if not self.use_rotary_emb:
                ...
                first_frame_pos_pad = pos_idx.repeat_interleave(adjacent_slices.shape[-2]//self.n_frames, dim=0)
            ...
            mask_adjacent_slices = ~torch.eye(self.n_frames, dtype=bool, device=hidden_states.device)
            mask_adjacent_slices = mask_adjacent_slices.repeat_interleave(adjacent_slices.shape[-2]//self.n_frames, dim=1)
            mask_hidden_states = torch.ones((mask_adjacent_slices.shape[0], mask_adjacent_slices.shape[0]), dtype=torch.bool, device=hidden_states.device)
            attention_mask = torch.cat((mask_hidden_states, mask_adjacent_slices), dim=-1).unsqueeze(0).repeat_interleave(hidden_states.shape[0], dim=0)
...
class BasicLinearTransformerBlock3D(nn.Module):
    ...
    def __init__(self, in_channels, dim, n_heads, d_head, dropout=0.0, context_dim=None, image_space = False, gated_ff=True, checkpoint=True, temporal=False, causal=False, attention_bias = False, upcast_attention = False, cross_attention_only = False):
        super().__init__()
        self.attn1 = None
        if not cross_attention_only:
            self.attn1 = CrossAttention(
                query_dim=in_channels, heads=n_heads, dim_head=d_head, dropout=dropout, bias=attention_bias, upcast_attention=upcast_attention, temporal = temporal, causal = causal
            )  # is a self-attention
...
class LinearTransformer3D(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0.0, context_dim=None, image_space = False, temporal=[False,], causal=[False,], cross_attention_only = False):
        super().__init__()
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicLinearTransformerBlock3D(in_channels, inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim, temporal=temporal[d], causal=causal[d], image_space = image_space, cross_attention_only=cross_attention_only)
    ...
    def forward(self, hidden_states, context=None, mask=None, return_attn = False):
        ...
        if self.causal:
            if self.temporal:
                ...
                i, j = query.shape[-2]//77,key.shape[-2]//77
                mask = torch.ones((i, j), dtype = torch.bool, device = query.device).tril(j - i)
                mask = mask.repeat_interleave(77, dim=0)
                mask = mask.repeat_interleave(77, dim=1)

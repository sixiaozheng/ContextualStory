...
class BasicConditionalTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if condition_on_first_frame:
            ...
            first_frame_hidden_states = rearrange(norm_hidden_states, '(b f) d h -> b f d h', f=self.n_frames) #[:, 0, :, :]
            # first_frame_hidden_states = rearrange(norm_hidden_states, '(b f) d h -> b f d h', f=self.n_frames)[:, 0, :, :]
            first_frame_hidden_states = rearrange(first_frame_hidden_states, 'b f (h w) c -> (b f) h w c', h=input_height, w=input_width)
            ...
            adjacent_slices = first_frame_windows[:, :, :, :, mask] # [15, 320, 32, 32, 8] [3, 320, 32, 32, 8]
            attn_output = self.attn1(
                norm_hidden_states,

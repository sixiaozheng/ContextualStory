...
from .videoldm_attention import  LinearTransformer3D
...
class VideoLDMUNet3DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        # additional
        use_temporal: dict = {"temporal_attn": True, "temporal_conv": True}, #temporal_attn, temporal_conv
        n_frames: int = 8,
        n_temp_heads: int = 8,
        ...
        use_text_cross_attention: Optional[str] = None,
        use_motion_condition: bool = False,
        num_layers_FStext: int = 4,
        zero_init_storyflow_adapter: bool = True,
        zero_init_FSText: bool = True,
        FStext_layer_depth: int = 2,
        FStext_layer_temporal: list = [False, True],
        FStext_layer_causal: list = [False, False],
    ):
        super().__init__()
        ...
        self.use_motion_condition = use_motion_condition
        if self.use_motion_condition:
            print("use_motion_condition")
            self.msim_embedding = nn.Sequential(
                        nn.Linear(timestep_input_dim, time_embed_dim),
                        nn.SiLU(),
                        nn.Linear(time_embed_dim, time_embed_dim))
            if zero_init_storyflow_adapter:
                nn.init.zeros_(self.msim_embedding[-1].weight)
                nn.init.zeros_(self.msim_embedding[-1].bias)

        self.use_text_cross_attention = use_text_cross_attention
        if self.use_text_cross_attention=="FSText":
            print("init FSText adapter")
            self.text_adapter = FSTextTransformer(num_frames = n_frames, num_layers = num_layers_FStext, zero_init_FSText=zero_init_FSText, depth=FStext_layer_depth, temporal=FStext_layer_temporal, causal=FStext_layer_causal) # default 4
        elif self.use_text_cross_attention!="":
            print("init text adapter")
    ...
    def get_motion_embedding(self, batch, f, motion_cond):
        dim = self.time_embedding.linear_1.in_features
        # batch = motion_cond.shape[0]
        if f > 1:
            if motion_cond.size(1) != f:
                motion_embedding = self.sinusoidal_embedding(motion_cond.flatten(0, 1), dim).view(-1, f-1, dim) # [6,4,320]
                motion_embedding = torch.nn.functional.interpolate(motion_embedding.transpose(1, 2), size=(f), mode='linear').transpose(1, 2) # [6,5,320]
            else:
                motion_embedding = self.sinusoidal_embedding(motion_cond.flatten(0, 1), dim).view(-1, f, dim)
            motion_embedding = self.msim_embedding(motion_embedding).flatten(0, 1) # [30,1280]
            if batch==motion_embedding.shape[0]:
                return motion_embedding
            else:
                return repeat(motion_embedding, "f c -> (b f) c", b=batch//f)
            return 
        else:
            return self.msim_embedding(self.sinusoidal_embedding(motion_cond, dim))
    ...
    def forward(
        ...
        motion_cond: Optional[torch.Tensor] = None,
        mode: Optional[str] = "train",
        do_classifier_free_guidance: Optional[str] = "text",
        ...
        if encoder_hidden_states is not None:
            if mode=="train":
                encoder_hidden_states_ori = encoder_hidden_states[0]
                encoder_hidden_states_pooled = encoder_hidden_states[1]
                null_text_encoder_hidden_states = encoder_hidden_states[2]
                is_null_text = encoder_hidden_states[3]
            elif mode=="test":
                text_embeddings, text_embeddings_pooled = encoder_hidden_states[0], encoder_hidden_states[1]
                encoder_hidden_states_pooled = text_embeddings_pooled
                if do_classifier_free_guidance == "text":
                    uncond_embeddings, encoder_hidden_states_ori = text_embeddings.chunk(2)
                elif do_classifier_free_guidance == "both":
                    uncond_embeddings, uncond_embeddings_, encoder_hidden_states_ori = text_embeddings.chunk(3)

            if self.use_text_cross_attention=="text_adapter":
                mask = ~torch.eye(video_length, dtype=bool, device=sample.device)
                mask = mask.repeat_interleave(encoder_hidden_states_ori.shape[-2], dim=0)
                mask = mask.repeat_interleave(encoder_hidden_states_ori.shape[-2], dim=1)
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "(b f) n c -> b (f n) c", f=video_length)
                mask = mask.unsqueeze(0).expand(encoder_hidden_states_ori.shape[0], -1, -1)
                    
                encoder_hidden_states_ori_ca = self.text_adapter(encoder_hidden_states_ori, encoder_hidden_states_ori, attention_mask=mask) #, attention_mask=mask
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori_ca, "b (f n) c -> (b f) n c", f=video_length)
            elif self.use_text_cross_attention=="text_adapter_wo_mask":
                # mask = ~torch.eye(video_length, dtype=bool, device=sample.device)
                # mask = mask.repeat_interleave(encoder_hidden_states_ori.shape[-2], dim=0)
                # mask = mask.repeat_interleave(encoder_hidden_states_ori.shape[-2], dim=1)
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "(b f) n c -> b (f n) c", f=video_length)
                ...
                encoder_hidden_states_ori_ca = self.text_adapter(encoder_hidden_states_ori, encoder_hidden_states_ori) #, attention_mask=mask
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori_ca, "b (f n) c -> (b f) n c", f=video_length)
            elif self.use_text_cross_attention=="text_adapter_pooler":
                encoder_hidden_states_pooled = rearrange(encoder_hidden_states_pooled, "(b f) c -> b f c", f=video_length)
                    
                encoder_hidden_states_pooled = repeat(encoder_hidden_states_pooled, "b f c -> (b g) f c", g=video_length)
                encoder_hidden_states_ori = self.text_adapter(encoder_hidden_states_ori, encoder_hidden_states_pooled) #, attention_mask=mask
            elif self.use_text_cross_attention=="Word-Level_SV_S_pooled":
                encoder_hidden_states_pooled = rearrange(encoder_hidden_states_pooled, "(b f) c -> b f c", f=video_length)
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "(b f) n c -> b (f n) c", f=video_length)
                query = encoder_hidden_states_pooled
                key = encoder_hidden_states_ori
                value = encoder_hidden_states_ori
                attn_weight = query @ key.transpose(-2, -1)
                attn_weight = torch.softmax(attn_weight, dim=-1)
                encoder_hidden_states_ori = attn_weight @ value
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "b f c -> (b f) 1 c")
            elif self.use_text_cross_attention=="Word-Level_SV":
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "(b f) n c -> b (f n) c", f=video_length)
                query = encoder_hidden_states_ori
                key = encoder_hidden_states_ori
                value = encoder_hidden_states_ori
                attn_weight = query @ key.transpose(-2, -1)
                attn_weight = torch.softmax(attn_weight, dim=-1)
                encoder_hidden_states_ori = attn_weight @ value
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "b (f n) c -> (b f) n c", f=video_length)
            elif self.use_text_cross_attention=="FSText":
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "(b f) n c -> b f n c", f=video_length)
                encoder_hidden_states_ori = self.text_adapter(context=encoder_hidden_states_ori) # [40, 77, 1024]
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "b f n c -> (b f) n c")

            if mode=="train":
                encoder_hidden_states_ori = rearrange(encoder_hidden_states_ori, "(b f) n c -> b f n c", f=video_length)
                for idx, is_null_text_ in enumerate(is_null_text):
                    if is_null_text_:
                        encoder_hidden_states_ori[idx] = null_text_encoder_hidden_states
                ...
                if do_classifier_free_guidance == "text":
                    encoder_hidden_states = torch.cat([uncond_embeddings, encoder_hidden_states_ori])
                elif do_classifier_free_guidance == "both":
                    encoder_hidden_states = torch.cat([uncond_embeddings, uncond_embeddings_, encoder_hidden_states_ori])
        ...
        emb = self.time_embedding(t_emb, timestep_cond) # [30, 1280]

        if self.use_motion_condition:
            emb = emb + self.get_motion_embedding(sample.shape[0], video_length, motion_cond) # 1, 32, [1,31]
    ...
    def __init__(
        ...
        for n in range(num_layers):
            self.trf_blocks.append(LinearTransformer3D(in_channels = in_channels, n_heads = n_heads, d_head = d_head,\
                depth = depth, context_dim = cross_attention_dim, temporal = temporal, causal = causal))
        self.norm = nn.LayerNorm(out_channels)

        if zero_init_FSText:
            self.trf_blocks[-1].transformer_blocks[-1].ff.net[-1] = zero_module(self.trf_blocks[-1].transformer_blocks[-1].ff.net[-1])
